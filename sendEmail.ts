...
class GmailSender {
  ...
  private async ensureAuthentication(): Promise<void> {
    ...
    try {
      ...
      if (!clientId || !clientSecret || !redirectUri) {
        ...
        console.error('Please ensure GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REDIRECT_URI are set in your .env file');
        console.error('Copy .env.example to .env and fill in your credentials from Google Cloud Console');
        return;
      }
  ...
  private parseEmailTemplate(templateFile: string): ParsedEmail | null {
    ...
    // Use default from email if not specified and available in env
    if (!headers.From && process.env.DEFAULT_FROM_EMAIL) {
      headers.From = process.env.DEFAULT_FROM_EMAIL;
    }
  ...
  public async sendEmail(templateFile: string = 'email.html.eml'): Promise<boolean> {
    ...
    try {
      ...
      const decodedMessage = Buffer.from(raw.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8');
